...
import { type Room, type RoomStatus, type UrgencyLevel } from "@/types/requestForm";
...
function UrgencySelector({ selected, onChange }: UrgencySelectorProps) {
    ...
    return (
        <div className="flex items-center gap-6" role="radiogroup">
            {levels.map((level) => (
                <label key={level.value} className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                        type="radio"
                        name="urgency"
                        value={level.value}
...
export default function FacultyRequestForm() {
    ...
    const set = (field: keyof FacultyFormState, value: string) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const createResponse = await fetch(`${BASE_URL}/api/requests/faculty`, {
                method: "POST",
            ...
            fetch(`${BASE_URL}/api/requests/faculty/trigger-flow`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    requestId: mongoId,
            ...
            navigate(`/status/faculty?requestId=${mongoId}`);
        } catch (err) {
    ...
    const inputBase =
        "w-full border px-3 py-2.5 text-sm text-[#1a1a1a] placeholder:text-[#d6d6d6] focus:outline-none focus:ring-1 focus:ring-[#064db6]/30 transition-all";
    const inputBorder = (field: keyof FacultyFormErrors) =>
        errors[field] ? "border-[#ed3a30] focus:border-[#ed3a30]" : "border-[#cbd5e1] focus:border-[#064db6]";
    const errorMsg = (field: keyof FacultyFormErrors) =>
        errors[field] ? <p className="text-xs text-[#ed3a30] mt-1">{errors[field]}</p> : null;
    const fieldLabel = (text: string, fieldId: string) => (
        <label htmlFor={fieldId} className="block text-xs font-black text-[#1a1a1a] uppercase tracking-widest mb-1.5">
            {text}
        </label>
    );
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-[#ffc107] text-xs mt-0.5 font-medium">
                                Book a consultation room and wait for strand head approval.
                            </p>
                        </div>
                    </div>
                ...
                <div className="bg-white border border-[#cbd5e1] px-5 py-6 space-y-5">
                            ...
                            <div className="absolute z-10 w-full mt-1 bg-white border border-[#cbd5e1] shadow-md overflow-hidden">
                                {filteredTeachers.map((t) => (
                                    <button
                                        key={t}
                                        type="button"
                                        className="w-full text-left px-4 py-2.5 text-sm text-[#1a1a1a] hover:bg-[#064db6]/5 transition-colors"
                                        onClick={() => {
                                            set("teacher", t);
                                            setTeacherQuery(t);
                                            setShowTeacherDropdown(false);
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <select
                                ...
                                className={`w-full border px-3 py-2.5 text-sm appearance-none bg-white focus:outline-none focus:border-[#064db6] focus:ring-1 focus:ring-[#064db6]/30 transition-all ${form.strand ? "text-[#1a1a1a]" : "text-[#d6d6d6]"} ${inputBorder("strand")}`}
                            >
                                <option value="" disabled>
                                    Select Strand
                                </option>
                                {strands.map((s) => (
                                    <option key={s} value={s} className="text-[#1a1a1a]">
                                        {s}
                                    </option>
                                ))}
                            </select>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={4}
                            className={`w-full border px-3 py-2.5 text-sm text-[#1a1a1a] placeholder:text-[#d6d6d6] resize-none focus:outline-none focus:border-[#064db6] focus:ring-1 focus:ring-[#064db6]/30 transition-all ${inputBorder("reason")}`}
                        />
                        {errorMsg("reason")}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label htmlFor="startTime" className="block text-[11px] font-bold uppercase tracking-widest text-[#4f4f4f] mb-1">
                                    Start Time
                                </label>
                                <input
                                    id="startTime"
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                    {startTime && <p className="text-[11px] text-[#4f4f4f]">{formatInputTimeLabel(startTime)} PST</p>}
                                    <button
                                        type="button"
                                        onClick={() => {
                            ...
                            <div>
                                ...
                                />
                                {endTime && <p className="text-[11px] text-[#4f4f4f] mt-1">{formatInputTimeLabel(endTime)} PST</p>}
                            </div>
                        </div>
                        {selectedTimeRange && (
                            <p className="mt-0 text-xs font-semibold" style={{ color: "var(--secondary)" }}>
                                Selected time: {selectedTimeRange}
                            </p>
                        )}
                        {form.teacher && activeBookedTimes.length > 0 && (
                            <div className="mt-2">
                                <p className="text-xs font-bold text-[#4f4f4f]">
                                    {form.teacher} has {activeBookedTimes.length} active booked consultation window
                                    {activeBookedTimes.length === 1 ? "" : "s"}:
                                </p>
                                <ul className="mt-1 text-xs font-bold text-[#4f4f4f] list-disc pl-5 space-y-0.5">
                                    {activeBookedTimes.map((slot) => (
                                        <li key={slot} style={{ color: "var(--destructive)" }}>
                                            {slot}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-3 text-[10px] font-semibold text-[#4f4f4f]">
                                <span className="flex items-center gap-1">
                                    <span className="w-2 h-2 rounded-full bg-[#ffef5f] inline-block" />Reserved
                                </span>
                                <span className="flex items-center gap-1">
                                    <span className="w-2 h-2 rounded-full bg-[#31ac52] inline-block" />Available
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="bg-[#f5f6fa] px-4 py-3 border border-[#cbd5e1]">
                        ...
                        <ul className="text-xs text-[#4f4f4f] space-y-1 list-disc pl-4">
                            ...
                            <li>Indicate urgency honestly to help strand heads prioritize requests.</li>
                            <li>Submissions are subject to strand head approval.</li>
                    ...
                    <button
                        ...
                        className="w-full bg-[#ffc107] hover:bg-[#ffd41c] active:scale-[0.98] text-[#002f73] font-black text-sm tracking-widest py-4 transition-all duration-150 uppercase"
                    >
                        Submit Form
                    </button>
                </div>
